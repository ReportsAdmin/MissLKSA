...
(select ord.*, fulfil.time_to_fulfill, deliv.Delivered_quantity, deliv.Delivered_Orders, {{Country}} Halo_Country from
(
...
(select
 ...
 from `{{magento_transactionnew}}` trans
 left join  `{{fOrders}}` ord
...
From `{{magento_transactionnew}}` ord
left join `{{magento_address}}` address
